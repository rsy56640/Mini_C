	...
	namespace supporters {
		...
		bool rIsInUnminusableSituation(vector<token_info> &r) {
			...
			return std::visit([&rb](auto&& r)->bool {
				...
				if constexpr (std::is_same_v<t, std::decay_t<identifier>>)
					return true;
				else if constexpr (std::is_same_v<t, std::decay_t<numeric_t>>)
					return true;
				else if constexpr (std::is_same_v<t, std::decay_t<type>>)
